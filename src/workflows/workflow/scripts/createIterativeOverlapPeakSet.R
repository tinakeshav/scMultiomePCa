...
#metadata <- 'peaks/corces_meta_filerows_colnames.tsv'
#macs2_dir <- 'peaks/'
#out_dir <- 'merged_peaks/'
#end <- '_summits.bed'
#blacklist_bed <- '/cluster/home/tkeshava/common/ENCFF356LFX.bed'
#genome <- 'hg38' 
#spm <- 2 
#selectionRules <- '1' 
#extend <- 250
...
extendedPeakSet <- function(df, BSgenome = NULL, blacklist = NULL, extend = 250, scorePerMillion = 2, selectionRules = "(n+1)/2"){
  ...
  print('groupSEList')
    ...
    grList <- lapply(seq_along(grList),function(x){
      ...
      print('saving grList')
      saveRDS(grList, paste0(out_dir, i,'grList.rds'))
